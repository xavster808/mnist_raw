...
impl Model {
    ...
    pub fn evaluate(&self, images: &[Image], labels: &[Label]) -> f32 {
        ...
        max.iter()
            .zip(labels)
            .fold(0, |a, (b, c)| a + (*b == c.num as usize) as usize) as f32
            / labels.len() as f32
    }
    ...
    pub fn update_batch(
        ...
        for (i, layer) in self.layers.iter_mut().enumerate() {
            ...
            layer.biases = layer
                .biases
                .subtract(&nabla_b[i].scale(learning_rate))
                .unwrap();
        }
    }

    // Using the quadratic cost function 0.5(a - y)^2
    fn backprop<T: ActivationFunction>(
        &self,
        ...
        let mut error = output
            .subtract(expected_output)
            .unwrap()
            .hadamard(&T::prime(&preactivations[preactivations.len() - 1]))
            .unwrap();
        nabla_w.push(
            error
        ...
        for l in 1..num_layers {
            error = self.layers[num_layers - l]
                .weights
                .transpose()
                .multiply(&error)
                .unwrap()
                .hadamard(&T::prime(&preactivations[num_layers - 1 - l]))
                .unwrap();
            nabla_w.push(
                error
                    .outer(&activations[activations.len() - l - 2])
                    .unwrap()
                    .scale(1.0 / n),
            );
            nabla_b.push(error.sum_along_rows().scale(1.0 / n));
